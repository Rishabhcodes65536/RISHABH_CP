...
void dosomething()
{
  ...
  map<int, int> mp;
  vector<vector<int>> ini(n + 2), oti(n + 2);
  vector<int> indeg(n + 1), indegg;
  rep(i, 0, n)
  {
    int k;
    cin >> k;
  ...
  {
    indegg = indeg;
    int cnt = 0, travelans = 0, playans = 0;
    int ptr = curr;
    ...
    ptr = (ptr + 1) % 3;
    // cout << curr<<" "<<travelans << " " << playans << endl;
    }
  res = min(res, travelans + playans);
  }
cout << res << endl;
}
