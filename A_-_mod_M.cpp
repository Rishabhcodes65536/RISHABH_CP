...
int b[n];
copy(a,a+n,b);
vector <int> kuchtologkahenge;
int grip=0,drip=0,prip=0;
sort(a,a+n);
if(n==1){
    cout<<1<<endl;
    return;
...
rep(i,1,n){
    kuchtologkahenge.pb(a[i]-a[i-1]);
}
rep(i,0,sz(kuchtologkahenge)){
    prip=__gcd(prip,kuchtologkahenge[i]);
}
/*
rep(i,0,n){
    grip=__gcd(a[i],grip);
...
rep(i,0,n){
    drip=__gcd(a[i]-1,drip);
}*/
if(prip==1){cout<<2<<endl;}
else{cout<<1<<endl;}
}
