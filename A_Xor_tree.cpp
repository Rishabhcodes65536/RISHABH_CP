...
void recursive(int i,vector <int>& init,vector <int>& want,vector <vector <int>>& g,vector <int>& vis,int state,int height,vector <int> &res){
    vis[i] = 1;
    //state 0 -3 00 01 10 11
    //need[0]=0 want[0]=1
    //00 01
    init[i] ^= (((1 << height) & state) > 0 ? (1):0);
    if(init[i]!=want[i]){
        state ^= (1 << height); 
        res.pb(i+1);
    }
    for(auto &r:g[i]){
        if(!vis[r]){
            recursive(r, init, want, g, vis, state, height ^ 1, res);
        }
    }
}
...
int state = 0;
recursive(0, init, want, g,visited,0,0,res);
cout << sz(res) << endl;
rep(i,0,sz(res)){
    cout << res[i] << endl;
}
