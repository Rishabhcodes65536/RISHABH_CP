...
return binpow(b,mod-2LL,mod)%mod;
}
...
int n;
cin>>n;
 int a=((n*(n+1))%M*(2*n+1)%M*(inv_mod(3LL,M))%M)%M;
// int a= (((n*(n+1))%M*(2*n+1))%M*inv_mod(3,M))%M;
int b=((n*(n+1))%M*inv_mod(2LL,M))%M;
int c=(((a-b+M)%M)*(2022))%M;
// cout<<a<<" "<<b<<" "<<c<<endl;
cout<<c<<endl;
}
signed main(){
