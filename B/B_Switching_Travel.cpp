...
bool flag=0;
void dfs(vector <vector <int>>& g,vector <int>& col,vector <int>& visited,int i,int start,bool more){
if(visited[start]>=1){
    flag=1;
    return;
}
...
for(auto h:g[i]){
    int color=col[i-1];
    if(col[h-1]!=color && !visited[h]){
        col[i-1]=1-col[i-1];
        dfs(g,col,visited,h,start,1);
        col[i-1]=1-col[i-1];
    }
}
visited[i]--;
