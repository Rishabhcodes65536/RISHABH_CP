...
for(int i = 0; i < n; i++){
    vep.push_back({x[i], speed[i]});
}
sort(vep.begin(), vep.end());
int res = 0;
long double mini_time = INT_MAX,vel=0;
per(i,n-1,0){
    long double xx = vep[i].ff;
    long double tt = vep[i].ss;
    long double this_time=(long double)xx/(long double)tt;
    // cout << xx << " " << tt << endl;
    // cout << this_time << " " << maxi_time << endl;
    if(this_time>mini_time){
        continue;
    }
    else{
        mini_time = this_time;
