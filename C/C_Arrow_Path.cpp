...
bool recursive(int n,vector <vector <int>>& dp,vector <string>& ves,vector <vector <int>>& vis,int i,int j){
if(i==1 && j==n-1){
    return true;
}
if(dp[i][j]!=-1){
...
if(j-1>=0){
    ...
    left|=recursive(n,dp,ves,vis,i,j-1+upd);
}
if(j+1<n){
    int upd=0;
    if(ves[i][j+1]=='>'){
    ...
    right|=recursive(n,dp,ves,vis,i,j+1+upd);
}
if(i-1>=0){
    int upd=0;
    if(ves[i-1][j]=='>'){
    ...
    down=recursive(n,dp,ves,vis,i-1,j+upd);
}
if(i+1<2){
    int upd=0;
    if(ves[i+1][j]=='>'){
    ...
    up=recursive(n,dp,ves,vis,i+1,j+upd);
}
return dp[i][j]=((up||down||left||right) ? (1):(0));
}
...
vector <vector <int>> vis (2,vector <int> (n+2,0));

int i=0,j=0;
// cout<<recursive(n,dp,ves,vis,i,j)<<endl;
if(recursive(n,dp,ves,vis,i,j)){
    cout<<"YES"<<endl;
}
else{
    cout<<"NO"<<endl;
