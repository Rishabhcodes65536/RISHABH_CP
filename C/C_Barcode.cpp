...
void dosomething()
{
    ...
    vector<vector<vector <int>>> dp(m + 1, vector<vector <int>>(r + 1, vector <int> (2,-1)));
    function<int(int, int,int)> dfs = [&](int i, int u,int fuck_u_testcase) -> int
    {
        if (i == m)
        {
            if (u >= l && u <= r)
        ...
        if (dp[i][u][fuck_u_testcase] != -1)
        {
            return dp[i][u][fuck_u_testcase];
        }
        int incl = INT_MAX, excl = INT_MAX;

        if ((u < r && (fuck_u_testcase || i==0)) || (!fuck_u_testcase && u >=l))
        {
            incl = n-sp[i] + dfs(i + 1, (fuck_u_testcase==0) ? (1):(u+1),1);
        }
        // cout << u << " " << fuck_u_testcase << endl;
        if((u<r && !fuck_u_testcase) || (fuck_u_testcase && u>=l)){
            excl = sp[i] + dfs(i + 1, (fuck_u_testcase==0) ? (u+1):(1),0);
        }
        // cout << incl << " " << excl << " " << fuck_u_testcase << endl;
        return dp[i][u][fuck_u_testcase] = min(incl, excl);
    };
