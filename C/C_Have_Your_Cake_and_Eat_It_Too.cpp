...
int n;
cin>>n;
vector<vector<int>> vec(n, vector<int>(3, 0LL));
int sum = 0LL, flag = 0LL;
rep(j,0,3){
    rep(i,0,n){
        int a;
        cin >> a;
...
// cout << sum << endl;
int target = (sum+2LL)/3LL;
for (int i = 0; i < 3;i++){
    for (int j = 0; j < 3;j++){
        for (int k = 0; k < 3;k++){
            if (i != j && j != k && i!=k){
                int first = (i == 0) ? (0) : (j == 0 ? (1) : (2));
                int second = (i == 1) ? (0) : (j == 1 ? (1) : (2));
                int third = (i == 2) ? (0) : (j == 2 ? (1) : (2));
                int xx = 0LL,yy=0LL,zz=0LL;
                int ptr = 0LL,l1=0LL;
                while(ptr < n && xx<target){
                    xx += vec[ptr][first];
                    ptr++;
                }
                if(xx<target){
                ...
                int r3 = max(n-1,ptr - 1);
                // int r3 = ptr - 1;
                if(xx>=target && yy>=target && zz>=target){
                    vector<int> ans(6, 0LL);
                    ans[2 * first] = l1+1LL;
                    ans[2 * first+1] = r1+1LL;
                    ans[2 * second] = l2+1LL;
                    ans[2 * second+1] = r2+1LL;
                    ans[2 * third] = l3+1LL;
                    ans[2 * third + 1] = r3 + 1LL;
                    disp(ans);
                    return;
                }
