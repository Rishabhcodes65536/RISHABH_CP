...
int recursive(map <char,int>& mp,string &s,int j,char c,int flag,vector <vector <vector <int>>>& dp){

if(j<0){
    return 0;
}
if(dp[j][c-'A'][flag]!=INT64_MIN)      return dp[j][c-'A'][flag];

int excl=INT64_MIN,incl=INT64_MIN;
// vector <int> inclv(5,INT64_MIN);
...
if(!flag){
    ...
    for(int i=0;i<5;i++){  
        // if(s[j]!='A'+i){
            if('A'+i<c){
                incl=max(incl,-mp['A'+i]+recursive(mp,s,j-1,'A'+i>=
                c ? ('A'+i):(c),1,dp));
            }
            else{
                incl=max(incl,mp['A'+i]+recursive(mp,s,j-1,'A'+i>=c ? ('A'+i):(c),1,dp));
            }
        // }
    }
    // for(int i=0;i<5;i++){
    //     incl=max(incl,inclv[i]);
    // }
}
if(s[j]<c){
    excl=-mp[s[j]]+recursive(mp,s,j-1,s[j]>c ? (s[j]): (c),flag,dp);
}
else{
    excl=mp[s[j]]+recursive(mp,s,j-1,s[j]>=c ? (s[j]): (c),flag,dp);
}
return dp[j][c-'A'][flag]=max(incl,excl);
...
char c='A';
int j=s.size()-1;
vector <vector <vector <int>>> dp(s.size(),vector <vector <int>> (5,vector <int> (2,INT64_MIN)));
cout<<recursive(mp,s,j,c,flag,dp)<<endl;
