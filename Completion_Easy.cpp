...
void dosomething()
{
    ...
    for (int i = 0; i < (2 * n);i+=2){
        if(v[i]==0){
            ...
            if(v[i]>n){
                // auto it = vst.upper_bound(n / 2);
                // it--;
                // // int val = *it;
                // vst.erase(it);
                res = mod_mul(res, lo, M);
                lo--;
            }
            else{
                // auto it = vst.upper_bound(n / 2);
                // vst.erase(it);
                res = mod_mul(res, hi, M);
                hi--;
            }
        }
    }
    int pau = binpow(2LL, cnt, M);
    int pauu = mod_mul(FAC[cnt],FAC[cnt], M);
    int pauuu = mod_mul(pau, pauu,M);
    res = mod_mul(res, pauuu, M);
    cout << res << endl;
}
signed main()
{
