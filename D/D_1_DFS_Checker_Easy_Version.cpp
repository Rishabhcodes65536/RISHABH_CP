...
int h = log2(n + 1);
vector<int> level_sabke_niklenge(h,0),niklenge_uske_jo_khada_rahega(h,0);
int k = 1;
rep(i,0,h){
    for (int j = 0; j < (1 << i);j++){
        level_sabke_niklenge[i] += k++;
    }
}
int kk = 0;
rep(i,0,h){
    for (int j = 0; j < (1 << i);j++){
        niklenge_uske_jo_khada_rahega[i] += perm[kk++];
    }
}
int offset = 0;
rep(i,0,h){
    if(level_sabke_niklenge[i]!=niklenge_uske_jo_khada_rahega[i]){
        offset++;
    }
}
disp(level_sabke_niklenge);
disp(niklenge_uske_jo_khada_rahega);
rep(i, 0, q)
{
    int l, r;
    cin >> l >> r;
    ...
    int y = log2(l);
    int u = log2(r);
    int prev1 = niklenge_uske_jo_khada_rahega[y]==level_sabke_niklenge[y];
    int prev2 = niklenge_uske_jo_khada_rahega[u]==level_sabke_niklenge[u];
    niklenge_uske_jo_khada_rahega[y] -= (g - h);
    niklenge_uske_jo_khada_rahega[u] -= (h - g);
    if(prev1 && niklenge_uske_jo_khada_rahega[y]!=level_sabke_niklenge[y]){
        offset++;
    }
    if(prev2 && niklenge_uske_jo_khada_rahega[u]!=level_sabke_niklenge[u]){
        offset++;
    }
    if(!prev1 && niklenge_uske_jo_khada_rahega[y]==level_sabke_niklenge[y]){
        offset--;
