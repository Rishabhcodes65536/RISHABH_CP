...
int recursive(map <int,int>& mp,vector <int>& dp,int i,int mex){
    if(i==mex){
        return 0;
    }
    if(dp[i]!=-1){
    ...
    int ans = INT_MAX;
    for (int j =i ; j < mex;j++){
        ans = min(ans,mp[j] * i +recursive(mp,dp,j,mex));
    }
    return dp[i] = ans;
}
// 0 0 1 1 2
...
vector<int> dp(mex + 1, INT_MAX),sp(mex+1,-1);
dp[mex] = 0;
// sp[mex] = 0;
per(i,mex,0){
    rep(j,0,i){
        dp[j] = min(dp[j], dp[i] + i * (mp[j]-1));
    }
}
int res = recursive(mp, sp, 0 ,mex);
cout << res - mex << endl;
// cout << dp[0] << endl;
}

//000  1111 222222 3333333 4444444
