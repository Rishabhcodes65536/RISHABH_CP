...
int recursive(vector <pair <int,int>>& g,vector <vector <int>>& dp,int j,int state){

if(j==g.size()){
    return 0;
}

if(dp[j][state]!=INT64_MIN) return dp[j][state];

int incl=INT64_MIN,excl=INT64_MIN;
int x=g[j].ff;
int y=g[j].ss;
if(x==1){
    if(state==0){
        incl=y+recursive(g,dp,j+1,1);
    }
    excl=recursive(g,dp,j+1,state);
}

else{
    incl=y+recursive(g,dp,j+1,0);
    excl=recursive(g,dp,j+1,state);
}


return dp[j][state]=max(incl,excl);
}
...
vector <vector <int>> dp(n+5,vector <int> (2,INT64_MIN));
int j=0,state=0;
cout<<recursive(g,dp,j,state);
}
signed main(){
