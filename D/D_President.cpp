...
int recursive(vector <vector <int>>& g,vector <vector <int>>& dp,int i,int k){
    if(i>=g.size()){
        if(k>3e5){
            return 0;
        }
    ...
    int incl=1e15,excl=1e15;
    if(g[i][0]<g[i][1]){
      incl=(g[i][1]-g[i][0]+1LL)/2LL+recursive(g,dp,i+1,k+g[i][2]);
    }
    excl=0+recursive(g,dp,i+1,(g[i][0]<g[i][1] ? (k-g[i][2]):(k+g[i][2])));

    return dp[i][k]=min(incl,excl);
...
vector <vector <int>> dp(n+2,vector <int>(6e5+10,INT64_MIN));
int k=3e5,i=0;
cout<<recursive(g,dp,i,k)<<endl;
}
