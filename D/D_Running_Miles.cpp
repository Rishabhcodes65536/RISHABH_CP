...
int recursive(vector <int> &a,vector <vector <int>>& dp,int i,int k){
    if(i==a.size()){
        if(k>2){
            return 0;
        }
    ...
    int incl=-1,excl=-1;
    if(k==0){
        incl=i+a[i]+recursive(a,dp,i+1,k+1);
    }
    else if(k==1){
        incl=a[i]+recursive(a,dp,i+1,k+1);
    }
    else{
        incl=a[i]-i+recursive(a,dp,i+1,k+1);
    }

    excl=recursive(a,dp,i+1,k);
