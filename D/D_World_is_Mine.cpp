...
int recursive(int i,int g,vector <int>& temp,map <int,int>& mp,vector<vector<int>>& dp){
    if(i==temp.size()){
        return 0;
    }
    if(dp[i][g]!=-1){
    ...
    int barbad=INT_MAX, barbad_nahi=INT_MAX;
    if(i>g && i-g >= mp[temp[i]]){
        barbad = 0 + recursive(i + 1, g + mp[temp[i]]+1, temp, mp, dp);
    }
    barbad_nahi = 1 + recursive(i + 1, g, temp, mp, dp);

    return dp[i][g] = min(barbad, barbad_nahi);
}
...
int ans = 0;
int rem = (n - 1) / 2;
vector<vector<int>> dp(mp.size()+2, vector<int>(mp.size()+5, -1));
vector<int> temp;
for(auto &u:mp){
    temp.pb(u.ff);
}
...
int g = 0;
int turn = 0;
cout << recursive(i, g,temp, mp, dp)<<endl;
}
signed main(){
