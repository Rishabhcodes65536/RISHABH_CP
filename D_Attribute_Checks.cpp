...
void winterIsComing(){
    ...
    vector<vector<int>> dp(5003, vector<int>(5003, -1));
    indexed_set bada, chota;
    int y = 0,maxi=0;
    int u1 = 0, v1 = 0;
    dp[0][0] = 0;
    int res = 0;
    ...
    for (int i = 1; i <= m;i++){
        ...
        while(ind<w){
            if (v[ind] < 0)
            {
                auto it = chota.find_by_order(chota.order_of_key(abs(v[ind])));
                if (it != chota.end())
                {
                    chota.erase(it);
                }
            ...
            {
                auto it = bada.find_by_order(bada.order_of_key(abs(v[ind])));
                if (it != bada.end())
                {
                    chota.erase(it);
                }
        ...
        int ans=0
        for (int j = 0; j <= i;j++){
            int zz = bada.order_of_key(j+1);
            int zzz = chota.order_of_key((i-j)+1);
            ans= max(res, zz + zzz);
        }
    }
    cout << res << endl;
    ...
    //         if (v[i] < 0)
    //         {
    //             auto it = chota.find_by_order(chota.order_of_key(abs(v[i])));
    //             if(it!=chota.end()){
    //                 chota.erase(it);
    //             }
    //         }
    //         else if (v[i] > 0)
    //         {
    //             auto it = bada.find_by_order(bada.order_of_key(abs(v[i])));
    //             if (it != bada.end())
    //             {
    //                 chota.erase(it);
    //             }
    ...
    //         // int u = y,v=0;
    //         // while(v<=y){
    //         //     int r1 = bada.order_of_key(u + 1);
    //         //     int r2 = chota.order_of_key(v + 1);
    //         //     maxi = max(maxi, r1 + r2);
    //         //     u--;
    //         //     v++;
    //         // }
    ...
    //         int g = u1, h = v1+1;
    //         int r1 = bada.order_of_key(u + 1);
    //         int r2 = chota.order_of_key(w + 1);
    //         int r3 = bada.order_of_key(g + 1);
    //         int r4 = chota.order_of_key(h + 1);
    //         maxi = max(maxi, r1 + r2);
    //         maxi = max(maxi, r3 + r4);
    //         if(r1+r2>r3+r4){
    //             u1 = u;
    ...
    // }
    // cout << maxi << endl;
    // function<int(int, int,int)> dfs = [&](int i, int j,int it) -> int {
    //     if(it>=n){
    //         return 0;
    //     }
    //     if(dp[i][j]!=-1){
    ...
    //     }
    //     else if(v[it]>0){
    //         incl = (i >= v[it]) + dfs(i, j, it + 1);
    //     }
    //     else{
    //         excl = (j >= abs(v[it])) + dfs(i, j, it + 1);
    //     }
