...
void dosomething()
{
    ...
    while(l<=h){
        int mid = l + (h - l) / 2LL;
        auto poss = [&](int z,vector <int> y) -> bool
        {
            int minx = INT64_MAX;
            int off = 0;
            for (int i = 0; i < n;i++)
            ...
            {
                // res = min(res, z - minx);
                // res = min(res, z - min(minx, v.back()));
                return true;
            }
        };
        if(poss(mid,v)){  //chalu pehla fail hua to ek kaam karta hu high pe b-search lagata hu
            h = mid - 1;
        }
        else{
            l = mid + 1;
    ...
    while(ll<=hh){
        int mid = ll + (hh - ll) / 2;
        auto posss = [&](int z,vector <int> y) -> bool
        {
            int maxx = 0;
            int minx = INT64_MAX;
            for (int i = 0; i < n;i++)
            ...
            cout << endl;
            cout << mid << " " << maxx << " " << minx << " " << z << endl;
            return (maxx <= l && minx>=z);
        };
        if(posss(mid,v)){  //ab low pe try karta hu
            ll = mid + 1;
        }
        else{
            hh = mid - 1;
