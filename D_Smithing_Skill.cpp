...
void recursive(int i,int& j,int& mini,int& biggie,vector <pair <int,int>>& vep,vector <int>& dp,int& maxi){
    if(i>maxi){
        return;
    }

    for (j; j < vep.size() && vep[j].ff <= i;j++){
        if(vep[j].ss<mini){
            mini = vep[j].ss;
            biggie = vep[j].ff;
        }
    ...
    if(mini!=INT_MAX){
        int attempts = (i - biggie) / mini + 1;

        dp[i] = 2 * (attempts) + dp[i - attempts * mini];
    }
    recursive(i + 1, j, mini, biggie, vep, dp,maxi);
}
...
int j = 0,mini=INT_MAX,biggie=0;
recursive(0,j,mini,biggie,vep,dp,maxi);
int res = 0;
rep(i,0,m){
    if(c[i]>maxi){
        int attempts = (c[i] - biggie) / mini + 1;
        res += 2 * attempts + dp[c[i] - attempts * mini];
    }
    else{
        res += dp[c[i]];
    }
