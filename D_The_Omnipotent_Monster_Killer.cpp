...
void dosomething()
{
    ...
    function<int(int, int, int,int,int)> dfs = [&](int u, int p, int signal,int p1,int p2) ->int
    {
        if(dp[u][signal]!=-1){
            return dp[u][signal];
        }
        // cout << "Current root:--> " << u+1 << endl;
        // cout << g[u].size() << endl;
        if((g[u].size()<=1 && u!=0) || (u==0 && g[u].size()==0)){
            // cout << "leaf encountered"<<" "<<signal << endl;
            if(signal==1){
                return v[u];
            }
            return 0;
        ...
        int incl = INT64_MAX, excl=INT64_MAX;

        for(auto &h:g[u])
        {
            if(h!=p)
            {
                if (signal == 3) // green jhanda
                {
                    if(incl==INT64_MAX){
                        incl = 0;
                    }
                    incl += 0 + dfs(h, u, 1,u,p1);
                }
                if(signal==2){  //yellow jhanda
                    if (incl == INT64_MAX)
                    {
                        incl = 0;
                    }
                ...
                if(signal==1){  //red jhanda
                    if (excl == INT64_MAX){
                        excl = v[u];
                    }
                    excl += 0 + dfs(h, u, signal + 1, u, p1);
                }
                if(signal==2){  // yellow jhanda
                    if (excl == INT64_MAX)
                    {
                        excl = v[u] + min(v[u], v[p]);
                    }
                    excl += 0 + dfs(h, u, signal + 1, u, p1);
                }
                if(signal==0){ // root ka abstracted yellow jhanda(blue jhanda)
                    if (excl == INT64_MAX)
                    {
                        excl = v[u];
                    }
                    excl += 0 + dfs(h, u,2,u, p);
                }
            }
        }
        // cout << u+1<<" "<<incl << " " << excl << endl;
        return dp[u][signal] = min(incl, excl);
    };
