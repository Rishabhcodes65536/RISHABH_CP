...
int mod_mul(int a, int b, int m) {b = (b+m) % m; return (((a * b) % m) + m) % m;}
...
invec(v, n);
// int sum = SUM(v);
priority_queue<int, vector<int>, greater<int>> pq,gq;
sort(all(v));
for (int i = 0; i < n; i++) {
    pq.push(v[i]);
    gq.push(v[i]);
...
while(!gq.empty()){
    ...
    // cout << z << " ";
    gq.pop();
    res += mod_mul(z, (mod_sub(binpow(2ll, u, M),1LL,M)), M);
    res %= M;
}
// cout <<endl;
// res += mod_mul(curr_sum, (binpow(2ll, u, M) - 1LL), M);
...
rep(j,0,remm){
    ...
    pq.pop();
    q %= M;
    res += mod_mul(q,(binpow(2ll, u, M)),M);
    res %= M;
}
cout<<res<<endl;
}
