...
if(n==2){
    ...
    int res=1;
    rep(i,1,n-1){
        res=mod_mul(res,m-1,MOD);
        res%=MOD;
    }
    res
    
    // int x=mod_mul(m,m-1,MOD);
    // res=mod_mul(res,m-2,MOD)%MOD;
    // res=(res%MOD+(m-2)*res%MOD)%MOD;
    // res=(res%MOD+(m-1)%MOD)%MOD;
    cout<<res<<endl;
}
