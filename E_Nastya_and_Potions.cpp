...
int mod_div(int a, int b, int m) {a = a % m; b = b % m; return (mod_mul(a, mod_inv(b, m), m) + m) % m;}
 ...
 int dfs(vector<int>& dp,vector <int>& visited,int i,vector <vector <int>>& g,vector <int>& a){
    visited[i]=1;
    int x=INT64_MAX;
    for(auto y:g[i]){
        if(x==INT64_MAX)x=0;
        if(visited[y]==0){
            x+=dfs(dp,visited,y,g,a);
        }
        else{
            x+=dp[y];
        }
...
rep(i,0,n){
    if(visited[i]==0){
        dp[i]=min(dp[i],dfs(dp,visited,i,g,a));
    }
    res.pb(dp[i]);
}
disp(res);
