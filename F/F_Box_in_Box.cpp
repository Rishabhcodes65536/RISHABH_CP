...
bool compare1(const std::vector<int>& a, const std::vector<int>& b) {
    return a[1] < b[1];
}
bool compare2(const std::vector<int>& a, const std::vector<int>& b) {
    return a[2] < b[2];
}
void dosomething(){
int n;
...
// int upper=upper_bound(vec.begin(),vec.end(),vector <int>{3},compare)-vec.begin();
int l=0;
int h=n;
int ele=vec[0][0];
while(l<h){
...
if(vec[0][0]<vec[l][0] && vec[0][1]<vec[l][1] && vec[0][2]<vec[l][2]){
    cout<<"Yes"<<endl;
    return;
}
sort(vec.begin()+l,vec.end(),compare1);
// int upper1=upper_bound(vec.begin()+upper,vec.end(),vector <int>{vec[upper][1]},compare1)-vec.begin();
// l=h;
int low=l;
h=n;
ele=vec[l][1];
...
if(vec[low][0]<vec[l][0] && vec[low][1]<vec[l][1] && vec[low][2]<vec[l][2]){
    cout<<"Yes"<<endl;
    return;
}
low=l;
sort(vec.begin()+l,vec.end(),compare2);
// int upper2=upper_bound(vec.begin()+upper1,vec.end(),vector <int>{vec[upper1][2]},compare2)-vec.begin();
// l=h;
h=n;
ele=vec[l][2];
while(l<h){
