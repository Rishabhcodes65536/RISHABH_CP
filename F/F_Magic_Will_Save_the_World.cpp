...
int isdefeted(vector <int>& v,int x1,int y1,int i,map <pair<pair<int,int>,int>,int>& mp){
if(i==v.size()){
    return 1;
}
// cout<<mp[{{x1,y1},i}]<<endl;
if(mp[{{x1,y1},i}]>0)return mp[{{x1,y1},i}];

int flag1=-1,flag2=-1;
if(x1-v[i]>=0){
    flag1+=isdefeted(v,x1-v[i],y1,i+1,mp);
}
if(y1-v[i]>=0){
    flag2+=isdefeted(v,x1,y1-v[i],i+1,mp);
}
return mp[{{x1,y1},i}]=(flag1+flag2);
}
