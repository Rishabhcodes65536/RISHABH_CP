...
void winterIsComing()
{
    ...
    vector<vector <int>> dp(n + 1, vector <int> (2,INT64_MIN));
    function<int(int, int,int)> dfs = [&](int u, int p,int k) -> int
    {
        ...
        int incl = vv[u]-((k==0) ? (0) : (2*c)), excl = ((k==0) ? (0):(0));
        if(u==0){
            incl = vv[u];
        }
        for(int v:g[u])
