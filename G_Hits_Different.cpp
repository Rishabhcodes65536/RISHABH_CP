...
int vec[(int)1e6+10];
int ans[(long long)1e6+(long long)10LL];
// vector <int> dp(1e6+10,-1);
// int recursive(int vec[],int n,vector <int>& dp){
//     if(n<1 || svec[n]==0)return 0;

//     if(dp[n]!=-1)return dp[n];
//     svec[n]=0;
//     int lans=(vec[n-vec[n]]==vec[n]-1)?(recursive(vec,n-vec[n],dp)):(0);
//     int rans=(vec[n-vec[n]+1]==vec[n]-1)?(recursive(vec,n-vec[n]+1,dp)):(0);
//     // svec[n]=n*n;
//     int ans=n*n+lans+rans;
//     return dp[n]=ans;
// }
...
ans[0LL]=0LL;
rep(i,1LL,(long long)1e6+2LL){
    ans[i]=i*i;
    int lans=vec[i-vec[i]]==vec[i]-1LL ? (ans[i-vec[i]]):(0LL);
    int rans=vec[i-vec[i]+1LL]==vec[i]-1LL ? (ans[i-vec[i]+1]):(0LL);
    int g=i-vec[i]+1LL;
    int offset=0;
    if(lans>0LL && rans>0LL){
        offset=ans[g-vec[g]];
    }
    // cout<<lans<<" "<<rans<<" "<<i<<endl;
    ans[i]+=lans+rans-offset;
}
while(t--){
dosomething();
//Case #<<t<<'='<< ;
