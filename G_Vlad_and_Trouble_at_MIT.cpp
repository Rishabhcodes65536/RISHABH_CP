...
int recursive(int i,int z,vector <vector <int>>& dp,vector <vector <int>>& g,vector <int>& v,vector <int> &vis){
    if(dp[i][z]!=-1){
        return dp[i][z];
    }
    int res=0;
        ...
        if(!vis[h]){
            ...
            if(newstate==0){
                if(z==0){
                    ...
                    op1=0+recursive(h,z,dp,g,v,vis);
                    
                }
                else{
                    op1=0+recursive(h,z,dp,g,v,vis);
                    op2=1+recursive(h,0,dp,g,v,vis);
                }
            }
            else{
                if(z==0){

                    op1=0+recursive(h,newstate,dp,g,v,vis);
                }
                else if(z==newstate){
                    op1=0+recursive(h,z,dp,g,v,vis);
                }
                else{
                    op1=1+recursive(h,newstate,dp,g,v,vis);
                }
            }
            res+=min(op1,op2);
        }
...
cout<<recursive(start,z,dp,g,v,vis)<<endl;
}
signed main(){
