...
int recursive(int i,int u,int k,vector <vector <vector <int>>> &dp,vector <int>& v,int f,int g){
    if(i==v.size()){
        if(k==0 && u==2){
            return 0;
        }
    ...
    int incl = INT_MIN, excl = INT_MIN,incl2=INT_MIN,incl3=INT_MIN;
    if(u==0 && k>0){
        incl = v[i] + recursive(i + 1, 1, k - 1, dp, v,k<g ? (k==0 ? (0):(1)):(f),g);
    }
    if(u==1 && k>0){
        incl2 = 2 * v[i] + recursive(i+1, k == 2 ? (2) : (1),k-1,dp, v,k<g ? (k==0 ? (0):(1)):(f),g);
    }
    if(k==1){
        incl3 = v[i] + recursive(i + 1, 2, k - 1, dp, v,k<g ? (k==0 ? (0):(1)):(f),g);
    }
    excl = 0 + recursive(i + 1, u, k, dp, v,k<g ? (k==0 ? (0):(1)):(f),g);
    return dp[i][u][f]=max({incl,incl2,incl3,excl});
}
...
invec(v, n);
int f = 0;
vector<vector<vector <int>>> dp(n + 1, vector<vector <int>>(3+2,vector <int> (4,-1)));
cout << recursive(0, 0,k,dp,v,2,k)<<endl;
}
signed main(){
