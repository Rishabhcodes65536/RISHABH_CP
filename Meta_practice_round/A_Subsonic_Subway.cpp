...
void winterIsComing(int t)
{
    ...
    double maxi = a[0] == 0 ? (INT_MAX) : 1.0 / a[0], mini = b[0] == 0 ? (INT_MAX) : 1.0 / b[0];
    bool flag = 0;

    for (int i = 2; i-1< n;i++){
        
        double max_spd = a[i-1]==0 ? (INT_MAX) : ((long double)i/(1.0 * a[i - 1]));
        double min_spd = b[i-1] ==0 ? (INT_MAX) : ((long double)i/(1.0 * b[i - 1]));
        maxi = min(maxi, max_spd);
        mini = max(mini, min_spd);
        // cout << maxi << " " << mini << endl;
        if(maxi<mini){
            flag = 1;
            break;
