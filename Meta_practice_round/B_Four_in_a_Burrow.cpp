...
void winterIsComing(){
    ...
    int dp[6][6][6][6][6][6][6][2];
    int alice = 0, bob = 0;
    function<int(vector<int>, int)> dfs = [&](vector<int> state, int turn) -> int
    {
        auto check = [&](char c) -> int
        {
            //base condition koi nahi jeet raha ho
            //kab tak karenge 
            //flag kab set karenge jab exclusive alice ya bob jeete
            ...
            for (int i = 0; i < 6;i++)
            {
                for (int j = 0; j + 3 < 7;j++)
                {
                    if(ves[i][j]==c && ves[i][j+1]==c && ves[i][j+2]==c && ves[i][j+3]==c)
                    {
                        return 1;
                    }
                }
            ...
            for (int i = 0; i+3 < 6; i++)
            {
                for (int j = 0; j < 7; j++)
                {
                    if (ves[i][j] == c && ves[i+1][j] == c && ves[i+2][j] == c && ves[i+3][j] == c)
                    {
                        return 1;
                    }
                }
            ...
            for (int i = 0; i + 3 < 6; i++)
            {
                for (int j = 0; j+3 < 7; j++)
                {
                    if (ves[i][j] == c && ves[i + 1][j+1] == c && ves[i + 2][j+2] == c && ves[i + 3][j+2] == c)
                    {
                        return 1;
                    }
                }
        ...
        if(dp[state[0]][state[1]][state[2]][state[3]][state[4]][state[5]][state[6]][turn] !=-1){
            return;
        }
