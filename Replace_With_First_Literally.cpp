...
void winterIsComing()
{
    ...
    // // ab s ki size badi hogi yaha tak
    // int offset = s.size() - t.size();

    vector<vector<vector<int>>> dp(n + 1, vector<vector<int>>(m + 1, vector<int>(2, -1)));
    function<int(int, int, int)> dfs = [&](int u, int v, int w) -> int
    {
        if (u == n)
        {
            if (v == m)
        ...
        if (v < m && s[u] == t[v])
        {
            incl = 0 + dfs(u + 1, v + 1, 0);
        }
        excl = ((w == 0) + dfs(u + 1, v, 1));

        return dp[u][v][w] = min(incl, excl);
    };
    int res = dfs(0, 0, 0);
    if (res>= 1e4)
    {
