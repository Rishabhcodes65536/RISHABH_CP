...
int maxi=INT_MAX;
rep(k,0,uniq.size()-1){
    vector <int> u=mp[uniq[k]];
    vector <int> v=mp[uniq[k+1]];
    int i=0,j=0;
    while(i<u.size() && j<v.size()){
        maxi=min(maxi,abs(u[i]-v[j]));
    if(u[i]<v[j]){
        i++;
    }
    else if(u[i]>v[j]){
...
int closestAdjacentPositions(int N, vector<int> arr) {
    map<int, vector<int>> mp;
    for (int i = 0; i < N; i++) {
...
cout<<closestAdjacentPositions(n,arr)<<endl;
//Case #<<t<<'='<< ;
}
    return 0;
}