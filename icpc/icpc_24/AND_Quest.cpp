...
bool solve(int idx, int curr, int k, vector<int> &arr, vector<int> &subset, map<pair<int, int>, bool> &dp)
{
    if (curr<k){
        return false;
    }
    ...
    if (solve(idx + 1, curr & arr[idx], k, arr, subset, dp))
    {
        return dp[key] = true;
    }
    subset.pop_back(); 
    if (solve(idx + 1, curr, k, arr, subset, dp))
    {
        return dp[key] = true;
    }
...
void winterIsComing()
{
    ...
    // function<int(int, int)> recursive(int i, int x){
    //     if(x==k){
    //         felin = steps;
    //         return;
    //     }
    ...
    // auto get_mask_vec = [&](int l, int h) -> vector<pair<int, int>>
    // {
    //     int sz = h - l + 1;
    //     if (sz == 0)
    //     {
    ...
    // };
    // int res = 0;
    // vector<pair <int,int>> left = get_mask_vec(0, (n - 1) / 2);
    // vector<pair <int,int>> right = get_mask_vec((n - 1) / 2 + 1, n - 1);

    // // sort(all(left));
    // // sort(all(right));

    // for(auto &h:left){
    //     int msk_left = h.second;
    //     int g = h.first & k;
    //     auto b_search
    //     if(b_search())
    // }
