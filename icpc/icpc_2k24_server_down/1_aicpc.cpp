...
void winterIsComing()
{
    ...
    int ptr = 1;
    int k = 2;
    int g = (d + 1) / 2, gg = d / 2, h = 0;
    vector<pair<int, int>> edge;

    while (rem > 0 && h < g)
    {
        edge.push_back({ptr, k});
        ptr = k;
        k++;
        h++;
        rem--;
    ...
    if (leaves < 0 || rem < 0 || (leaves > 0 && rem == 0))
    {
        cout << -1 << endl;
        return;
    }
    ...
    // cout << rem << " " << leaves << endl;
    while (rem > 0)
    {
        if (rem == leaves)
        {
            edge.push_back({ones, k});
            k++;
            leaves--;
            rem--;
        }
    ...
    if (edge.size()<n - 1||leaves!=0||rem!=0)
    {
        cout <<-1<< endl;
    }
    else
