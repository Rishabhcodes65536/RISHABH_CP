...
void sjf(vector<vector<int>>v)
{
    ...
    //Stores BT and index of vec havector<int>g that BT after Sorting
...
void srtf(vector<vector<int>>v)
{
    ...
    int i,c=0,count=0;//i->Takes index of shortest ProceSS
    sort(v.begin(),v.end());
...
int main() 
{
    ...
    for(int i=0;i<n;i++)
    {cin>>ab[i][0]>>ab[i][1];ab[i][2]=i+1;}
    cout<<"Enter 0(Exit),1(FCFS),2(SJF),3(SRTF),4(Preamptive Priority),5(Non-Preamptive Priority),6(Round Robin) : ";
    int whichalgo;
    cin>>whichalgo;
    switch(whichalgo)
    {
        ...
        default:
        {
            cout<<"Enter Valid Number AlgorithM";
        }
    }cout<<"\n\n";
    }   
}